...
{
    public partial class MaintainForm : ChildForm
    {
        ILog log = log4net.LogManager.GetLogger(typeof(MaintainForm));
        ...
        public void BindSourceGrid(SourceGrid.Grid grid, DataTable data)
        {
            ...
            grid.Rows.Insert(0);
            grid[0, 0] = new SourceGrid.Cells.ColumnHeader("记录ID");
            grid[0, 1] = new SourceGrid.Cells.ColumnHeader("设备编号");
            grid[0, 2] = new SourceGrid.Cells.ColumnHeader("设备名称");
            grid[0, 3] = new SourceGrid.Cells.ColumnHeader("所属部门");
            grid[0, 4] = new SourceGrid.Cells.ColumnHeader("维护起始日期");
            grid[0, 5] = new SourceGrid.Cells.ColumnHeader("维护结束日期");
            grid[0, 6] = new SourceGrid.Cells.ColumnHeader("设备状态");
            grid[0, 7] = new SourceGrid.Cells.ColumnHeader("维护负责人");
            grid[0, 8] = new SourceGrid.Cells.ColumnHeader("维护说明");
            grid[0, 9] = new SourceGrid.Cells.ColumnHeader("维护计划编号");
            grid[0, 10] = new SourceGrid.Cells.ColumnHeader("计划起始日期");
            grid[0, 11] = new SourceGrid.Cells.ColumnHeader("计划维护天数");
            ...
            for (int i = 1; i < data.Rows.Count + 1; i++)
            {
                ...
                grid[i, 1] = new SourceGrid.Cells.Cell(data.Rows[i - 1][9], typeof(string));//设备编号
                grid[i, 2] = new SourceGrid.Cells.Cell(data.Rows[i - 1][10], typeof(string));//设备名称
                grid[i, 3] = new SourceGrid.Cells.Cell(data.Rows[i - 1][11], typeof(string)); //所属部门
                grid[i, 4] = new SourceGrid.Cells.Cell(data.Rows[i - 1][1], typeof(DateTime)); //维护起始日期
                grid[i, 5] = new SourceGrid.Cells.Cell(data.Rows[i - 1][2], typeof(DateTime)); //维护结束日期
                grid[i, 6] = new SourceGrid.Cells.Cell(data.Rows[i - 1][4], typeof(string));//设备状态
                grid[i, 7] = new SourceGrid.Cells.Cell(data.Rows[i - 1][3], typeof(string));//维护负责人
                grid[i, 8] = new SourceGrid.Cells.Cell(data.Rows[i - 1][5], typeof(string));//维护说明
                grid[i, 9] = new SourceGrid.Cells.Cell(data.Rows[i - 1][6], typeof(string));//维护计划编号
                grid[i, 10] = new SourceGrid.Cells.Cell(data.Rows[i - 1][7], typeof(DateTime));//计划起始日期
                grid[i, 11] = new SourceGrid.Cells.Cell(data.Rows[i - 1][8], typeof(int));//计划维护天数
                /*
                //将图片显示在单元格中
