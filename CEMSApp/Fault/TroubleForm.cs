...
{
    public partial class TroubleForm : ChildForm
    {
        ILog log = log4net.LogManager.GetLogger(typeof(TroubleForm));
        ...
        private void AccountForm_Load(object sender, EventArgs e)
        {
            ...
            ds_report = ds_trouble;//公共DS，用于报表输出
        ...
        public void BindSourceGrid(SourceGrid.Grid grid, DataTable data)
        {
            ...
            int[] ColumnWidth = new int[] { 40, 100, 100, 100, 100, 240, 240, 240, 240 };
            ...
            grid[0, 4] = new SourceGrid.Cells.ColumnHeader("事故时间");
            grid[0, 5] = new SourceGrid.Cells.ColumnHeader("事故经过");
            grid[0, 6] = new SourceGrid.Cells.ColumnHeader("事故原因");
            grid[0, 7] = new SourceGrid.Cells.ColumnHeader("事故损失");
            grid[0, 8] = new SourceGrid.Cells.ColumnHeader("对策措施");
            ...
            for (int i = 1; i < data.Rows.Count + 1; i++)
            {
                ...
                grid[i, 4] = new SourceGrid.Cells.Cell(data.Rows[i - 1][4], typeof(DateTime)); //事故时间
                grid[i, 5] = new SourceGrid.Cells.Cell(data.Rows[i - 1][5], typeof(string)); //事故经过
                grid[i, 6] = new SourceGrid.Cells.Cell(data.Rows[i - 1][6], typeof(string));//事故原因
                grid[i, 7] = new SourceGrid.Cells.Cell(data.Rows[i - 1][7], typeof(string));//事故损失
                grid[i, 8] = new SourceGrid.Cells.Cell(data.Rows[i - 1][8], typeof(string));//对策措施
        ...
        private void editButton_Click(object sender, EventArgs e)
        {
            ...
            TroubleEditForm tef = new TroubleEditForm(id, grid1[grid1.Selection.ActivePosition.Row, 2].ToString());
