...
{
    public partial class RepairForm : ChildForm
    {
        ILog log = log4net.LogManager.GetLogger(typeof(RepairForm));
        ...
        {
            int[] ColumnWidth = new int[] { 40, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 240, 240, 240 };
            if (grid.RowsCount > 0)
            {
                //清除原表格内容
            ...
            grid.Rows.Insert(0);
            grid[0, 0] = new SourceGrid.Cells.ColumnHeader("维修ID");
            grid[0, 1] = new SourceGrid.Cells.ColumnHeader("维修记录编号");
            ...
            grid[0, 6] = new SourceGrid.Cells.ColumnHeader("维修开始日期");
            grid[0, 7] = new SourceGrid.Cells.ColumnHeader("维修结束日期");
            ...
            grid[0, 11] = new SourceGrid.Cells.ColumnHeader("维修班组");
            grid[0, 12] = new SourceGrid.Cells.ColumnHeader("维修负责人");
            grid[0, 13] = new SourceGrid.Cells.ColumnHeader("维修前设备状况");
            grid[0, 14] = new SourceGrid.Cells.ColumnHeader("维修后设备状况");
            grid[0, 15] = new SourceGrid.Cells.ColumnHeader("处理记录");
            ...
            for (int i = 1; i < data.Rows.Count + 1; i++)
            {
                ...
                grid[i, 1] = new SourceGrid.Cells.Cell(data.Rows[i - 1][1], typeof(string));//维修记录编号
                ...
                grid[i, 6] = new SourceGrid.Cells.Cell(data.Rows[i - 1][6], typeof(DateTime));//维修开始日期
                grid[i, 7] = new SourceGrid.Cells.Cell(data.Rows[i - 1][7], typeof(DateTime));//维修结束日期
                ...
                grid[i, 11] = new SourceGrid.Cells.Cell(data.Rows[i - 1][11], typeof(string));//维修班组
                grid[i, 12] = new SourceGrid.Cells.Cell(data.Rows[i - 1][12], typeof(string));//维修负责人
                grid[i, 13] = new SourceGrid.Cells.Cell(data.Rows[i - 1][13], typeof(string));//维修前设备状况
                grid[i, 14] = new SourceGrid.Cells.Cell(data.Rows[i - 1][14], typeof(string));//维修后设备状况
                grid[i, 15] = new SourceGrid.Cells.Cell(data.Rows[i - 1][15], typeof(string));//处理记录
        ...
        private void DelButton_Click(object sender, EventArgs e)
        {
            ...
            if (grid1[grid1.Selection.ActivePosition.Row, 0] != null)
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    ...
                    flag = acc.deleteRepairById(grid1[grid1.Selection.ActivePosition.Row, 0].ToString());
                        if (flag)
                        {
